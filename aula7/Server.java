...
class ServerWorker2 implements Runnable {
    ...
    public ServerWorker2(Socket socket, ContactManager manager) {
        this.socket = socket;
    ...
    @Override
    public void run() {
        // implementar o servidor worker
        try{
            DataInputStream dis = new DataInputStream(this.socket.getInputStream());

            while(true){
                Contact newContact = Contact.deserialize(dis);
...
public class Server {
    ...
    public static void main (String[] args) throws IOException {
        ...
        while (true) {
            ...
            Thread worker = new Thread(new ServerWorker2(socket, manager));
